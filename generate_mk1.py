            ...
            if design['Mark1Config'] == config['Mark1Config']:
              ...
              for finish_price_index, finish_price in data[
                ...
                if finish_price['Parameter2'] == config[
                    'Mark1ConfigSKU'] and finish_price['Parameter1'] == finish[
                      'HelmetFinishSKU']:
                  finish_eur = float(finish_price['M1PriceEUR'])
                  finish_usd = float(finish_price['M1PriceUSD'])
                  finish_rmb = float(finish_price['M1PriceRMB'])

                if finish_price['Parameter1'] == config[
                    'Mark1ConfigSKU'] and finish_price['Parameter2'] == design[
                      'Mark1PriceCategory']:
                  design_eur = float(finish_price['M1PriceEUR'])
                  design_usd = float(finish_price['M1PriceUSD'])
                  design_rmb = float(finish_price['M1PriceRMB'])
